...
const LoadingScreen = () => (
  <div className="min-h-screen flex items-center justify-center" style={patternStyle}>
    <div className="flex flex-col items-center space-y-4 bg-white/80 p-8 rounded-2xl backdrop-blur-sm border border-pink-100 shadow-xl">
      ...
      <p className="text-pink-600 font-medium">Connecting to Love Cloud...</p>
    </div>
  </div>
);
...
const LandingPage = ({ onJoin }) => {
  ...
  // 🔐 THE VIP LIST
  const ACCOUNTS = {
    // CHANGE THESE TO YOUR REAL PREFERRED IDs
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 max-w-2xl mx-auto w-full">
        <div className="bg-white/90 p-8 rounded-3xl shadow-xl border border-pink-100 backdrop-blur-md w-full">
          <div className="mb-10 text-center">
            ...
            <h2 className="text-2xl md:text-3xl font-bold text-slate-800 leading-tight">{question.text}</h2>
          </div>
            ...
            <div className="grid grid-cols-5 gap-3">
              {[1, 2, 3, 4, 5].map((val) => (
                <button
                  ...
                  className="aspect-square rounded-2xl border-2 border-pink-100 hover:bg-pink-500 hover:text-white text-xl font-bold text-pink-400 transition-all focus:outline-none focus:ring-4 focus:ring-pink-200 transform hover:scale-105 shadow-sm"
                >
                  {val}
                </button>
              ))}
...
const Dashboard = ({ coupleId, partnerName, data, onStart, onLogout }) => {
  ...
  // 1. First, sort all data by date
  const allSortedData = useMemo(() => {
    return [...data].sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
  }, [data]);

  // 2. Then, filter based on selection
  const visibleData = useMemo(() => {
    if (filter === 'all') return allSortedData;
    if (filter === 'me') return allSortedData.filter(d => d.partner_name === partnerName);
    if (filter === 'partner') return allSortedData.filter(d => d.partner_name !== partnerName);
    return allSortedData;
  }, [allSortedData, filter, partnerName]);
    ...
    <div className="min-h-screen pb-12" style={patternStyle}>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-pink-100 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <button onClick={onStart} className="bg-pink-500 hover:bg-pink-600 text-white px-8 py-3 rounded-xl font-bold shadow-lg shadow-pink-200 transition-all flex items-center justify-center space-x-2 transform hover:-translate-y-0.5">
        ...
        <div className="flex justify-center">
          <div className="bg-white p-1 rounded-xl border border-pink-100 shadow-sm flex space-x-1">
            <button 
              ...
              className={`px-6 py-2 rounded-lg text-sm font-bold transition-all flex items-center space-x-2 ${filter === 'all' ? 'bg-pink-500 text-white shadow-md' : 'text-slate-500 hover:bg-pink-50'}`}
            >
            ...
            <button 
              ...
              className={`px-6 py-2 rounded-lg text-sm font-bold transition-all flex items-center space-x-2 ${filter === 'me' ? 'bg-pink-500 text-white shadow-md' : 'text-slate-500 hover:bg-pink-50'}`}
            >
            ...
            <button 
              ...
              className={`px-6 py-2 rounded-lg text-sm font-bold transition-all flex items-center space-x-2 ${filter === 'partner' ? 'bg-pink-500 text-white shadow-md' : 'text-slate-500 hover:bg-pink-50'}`}
            >
        ...
        {latest ? (
            ...
            <div className="flex flex-col md:flex-row gap-8 items-start relative z-10">
              ...
              <div className="bg-white rounded-3xl p-8 shadow-md min-w-[220px] flex flex-col items-center justify-center border-2 border-white/50">
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 w-full text-center">
                  <span className="text-slate-400 text-xs block font-medium">Last update by</span>
          ...
          <div className="bg-white rounded-3xl p-12 border-2 border-dashed border-pink-200 text-center">
            ...
            <p className="text-slate-500 max-w-sm mx-auto">
              {filter === 'partner' 
                ? "Your partner hasn't completed an assessment yet." 
                : "Complete your first relationship assessment to unlock insights."}
            </p>
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl p-6 border border-pink-100 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-bold text-slate-800 flex items-center">
                ...
                Satisfaction Trends ({filter === 'all' ? 'Combined' : filter === 'me' ? 'My Trends' : "Partner's Trends"})
              </h3>
            </div>
        ...
        {visibleData.length > 0 && (
          <div className="bg-white rounded-2xl border border-pink-100 shadow-sm overflow-hidden">
            ...
            <div className="divide-y divide-pink-50">
              {visibleData.slice().reverse().map((entry, idx) => (
